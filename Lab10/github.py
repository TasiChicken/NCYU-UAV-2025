...
step_finish = 3
step_decide_direction = 4
step_list = {0:'prepare',1:'step_correct',2:'step_follow_line_and_search',3:'step_finish', 4:'step_decide_direction'}
...
HSV_upper = [180, 255, 60]#[110,247,101]#[126,255,255]
HSV_bottom = [0,0,0]#[101,190,69]            #[97,233,232]
FRAME_DIVIDER = 5.05
        ...
        while True: 
            ...
            ##########################################################################
                ...
                if (current_state == 0 and is_flying):
                    ...
                    if (timer_enter_first_counter == 0):
                        ...
                        if (child_state == 0):
                            ...
                            if(avg_height>150):
                                timer_set = (150.0-avg_height)/BASIC_VUD*TIME_SET_ALAPHA+TIME_SET_BETA
                                drone.send_rc_control(0,0,BASIC_VUD,0)
                                print("down to 150 cm height")
                                timer_prev, timer_go = timer_count(time.time(), timer_prev, timer_set)
                            elif(avg_height<130):
                                timer_set = (avg_height-130.0)/BASIC_VUD*TIME_SET_ALAPHA+TIME_SET_BETA
                                drone.send_rc_control(0,0,-1*BASIC_VUD,0)
                                print("up to 130 cm height")
                                timer_prev, timer_go = timer_count(time.time(), timer_prev, timer_set)
                            else:
                                drone.send_rc_control(0,0,0,0)
                            if timer_go:
                ...
                #=======================================
                    ...
                    if target_idex != -1:
                        correct_ready = correct_v2(rvec, tvec, target_idex, 80,x_pid, y_pid, z_pid, yaw_pid)
                    else:
                        #TODO
                        #fly_around()
                        ################################
                        ...
                        ################################
                        if is_flying == True:
                            drone.send_rc_control(0, 0, 20, 0)      #Stop in the air
                        #now = time.ctime()
                ...
                #=======================================
                    ...
                    if target_idex != -1:
                        ...
                        if en_left and en_up:
                             print("T-junction detected!")
                             current_state = step_decide_direction
                             drone.send_rc_control(0, 0, 0, 0)
                             t_junction_counter += 1
                             timer_prev = time.time() # Reset timer for the maneuver
                             continue
                ...
                #=======================================
                #       Decide Direction at T-Junction
                #=======================================
                elif (current_state == step_decide_direction):
                    print(f"state: {step_list[current_state]}, counter: {t_junction_counter}")
                    
                    # First T-junction: Go UP (Forward)
                    if t_junction_counter == 1:
                        timer_set = 2.0 # Move for 2 seconds
                        drone.send_rc_control(0, 0, 20, 0) # Forward 20
                        print("1st Junction: Going UP")
                    
                    # Second T-junction: Go LEFT
                    elif t_junction_counter == 2:
                        timer_set = 2.0 # Move for 2 seconds
                        drone.send_rc_control(-20, 0, 0, 0) # Left 20
                        print("2nd Junction: Going LEFT")
                    
                    else:
                        # Default/Fallback
                        timer_set = 2.0
                        drone.send_rc_control(0, 0, 0, 0)
                        print(f"Junction {t_junction_counter}: Hovering")
                    ...
                    if timer_go:
                        current_state = step_follow_line_and_search
                        drone.send_rc_control(0, 0, 0, 0)
                        print("Maneuver complete, returning to line follow")
