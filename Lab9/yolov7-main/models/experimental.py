...
# Import the YOLO model class so it can be allow‑listed for safe deserialization
try:
    from models.yolo import Model as YoloModel  # type: ignore
...
def attempt_load(weights, map_location=None):
    ...
    for w in weights if isinstance(weights, list) else [weights]:
        attempt_download(w)
        # PyTorch 2.6+ defaults to weights_only=True which is safer but breaks
        # loading older YOLOv7 checkpoints that store full objects.
        # We first try the secure path with an allow‑listed YOLO Model class;
        # if that still fails, fall back to weights_only=False **only** for
        # trusted, local checkpoints.
        try:
            if (
                hasattr(torch.serialization, "add_safe_globals")
                and YoloModel is not None
            ):
                torch.serialization.add_safe_globals([YoloModel])
            ckpt = torch.load(
                w, map_location=map_location
            )  # default weights_only behavior
        except Exception:
            # Final fallback: allow full unpickling. Use only with trusted weights.
